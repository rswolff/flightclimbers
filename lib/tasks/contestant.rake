	...
	task :calculate_contestant_week_sums => :environment do 
		...
		contest.contest_weeks.each do |contest_week|
			...
			contest.contestants.each do |contestant|
				...
				contestant_week.up_flights = Measurement.select("sum(number_of_flights) as sum_number_of_flights, days.date").joins(:day).where(:direction => 'up', :day_id => days, :user_id => contestant.id).first.sum_number_of_flights || 0
				contestant_week.up_flights_extended_value = Measurement.select("sum(extended_value) as sum_extended_value, days.date").joins(:day).where(:direction => 'up', :day_id => days, :user_id => contestant.id).first.sum_extended_value || 0.0
				contestant_week.down_flights = Measurement.select("sum(number_of_flights) as sum_number_of_flights, days.date").joins(:day).where(:direction => 'down', :day_id => days, :user_id => contestant.id).first.sum_number_of_flights || 0
				contestant_week.down_flights_extended_value = Measurement.select("sum(extended_value) as sum_extended_value, days.date").joins(:day).where(:direction => 'down', :day_id => days, :user_id => contestant.id).first.sum_extended_value || 0.0
				contestant_week.total_flights = Measurement.select("sum(number_of_flights) as sum_number_of_flights, days.date").joins(:day).where(:day_id => days, :user_id => contestant.id).first.sum_number_of_flights || 0
				contestant_week.total_flights_extended_value = 	Measurement.select("sum(extended_value) as sum_extended_value, days.date").joins(:day).where(:day_id => days, :user_id => contestant.id).first.sum_extended_value || 0.0
